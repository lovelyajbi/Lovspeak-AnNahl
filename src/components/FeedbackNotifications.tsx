...
const FeedbackNotifications: React.FC<{ user: User | null }> = ({ user }) => {
  ...
  return <div className="relative">
    ...
    {open && <><div onClick={() => setOpen(false)} className="fixed inset-0 z-[99] sm:hidden bg-black/30 backdrop-blur-sm" /><div className="fixed sm:absolute z-[100] left-3 right-3 top-[68px] sm:left-auto sm:right-0 sm:top-10 sm:w-[360px] max-h-[min(78vh,32rem)] overflow-y-auto rounded-2xl bg-white dark:bg-gray-900 shadow-2xl border border-gray-100 dark:border-gray-700 p-4"><div className="flex justify-between gap-3 items-center"><div><b>Notifikasi</b><p className="text-[10px] text-gray-400 mt-0.5">Komentar dan tugas tersimpan di sini</p></div>{(unreadItems.length + unreadNotifications.length) > 0 && <button onClick={() => { void readAll(); void readNotifications(); }} className="text-xs text-lovelya-600 font-bold">Tandai dibaca</button>}</div>{(items.length || notifications.length) ? <div className="mt-3 space-y-3">{shownNotifications.map(item => <div key={item.id} className={`rounded-xl p-3 border ${item.readAt ? 'bg-gray-50 dark:bg-gray-800 border-transparent' : 'bg-lovelya-50/60 dark:bg-lovelya-900/20 border-lovelya-200 dark:border-lovelya-700'}`}><p className="text-[10px] font-black uppercase text-lovelya-600">{item.kind === 'assignment' ? 'Tugas baru' : 'Pesan admin'}</p><p className="mt-1 text-sm font-bold">{item.title}</p><p className="mt-1 text-xs whitespace-pre-wrap">{item.message}</p></div>)}{shownItems.map(item => <div key={item.id} className={`rounded-xl p-3 text-sm border ${item.readAt ? 'bg-gray-50 dark:bg-gray-800 border-transparent' : 'bg-lovelya-50/60 dark:bg-lovelya-900/20 border-lovelya-200 dark:border-lovelya-700'}`}><div className="flex justify-between gap-2"><p className="text-[10px] font-black uppercase text-lovelya-600">{item.scope === 'task' ? item.taskTitle || 'Task' : 'Komentar umum'}</p><span className="text-[10px] text-gray-400">{new Date(item.createdAt).toLocaleDateString('id-ID')}</span></div><p className="mt-1 whitespace-pre-wrap">{item.message}</p>{(replies[item.id] || []).map(replyItem => <div key={replyItem.id} className="mt-2 flex items-start gap-2 rounded-lg bg-white/70 dark:bg-gray-700 p-2 text-xs"><p className="flex-1"><b>{replyItem.authorName}:</b> {replyItem.message}</p><button onClick={() => removeReply(item.id, replyItem.id)} className="text-gray-400 hover:text-red-500" aria-label="Hapus balasan"><i className="fas fa-trash text-[10px]"></i></button></div>)}<div className="mt-2 flex gap-2"><input value={drafts[item.id] || ''} onChange={event => setDrafts(current => ({ ...current, [item.id]: event.target.value }))} placeholder="Balas komentar..." className="min-w-0 flex-1 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-xs"/><button onClick={() => reply(item.id)} className="rounded-lg bg-lovelya-600 px-2 text-xs font-bold text-white">Kirim</button></div><button onClick={() => removeFeedback(item.id)} className="mt-2 text-[10px] font-bold text-gray-400 hover:text-red-500"><i className="fas fa-trash mr-1"></i>Hapus komentar</button></div>)}{hiddenCount > 0 && <button onClick={() => setShown(current => current + 10)} className="w-full rounded-xl border border-dashed border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 py-2 text-[11px] font-black text-lovelya-600">Muat lebih banyak ({hiddenCount} tersisa)</button>}</div> : <p className="text-sm text-gray-400 mt-3">Belum ada notifikasi.</p>}</div></>}
  </div>;
};
