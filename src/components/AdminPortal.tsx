...
const AdminAssignmentsPanel: React.FC<{ users: AdminUser[]; adminUid: string; onMessage: (message: string) => void; initialRecipientIds?: string[] | null; onConsumePrefill?: () => void }> = ({ users, adminUid, onMessage, initialRecipientIds, onConsumePrefill }) => {
  ...
  return <section className="admin-card admin-table-card">
    ...
    <div className="admin-assignment-form">
      ...
      {recipientMode === 'selected' && <div className="admin-recipient-grid">{users.map(item => <label key={item.uid}><input type="checkbox" checked={recipientIds.includes(item.uid)} onChange={() => toggleRecipient(item.uid)} />{item.name}<small>{item.email || 'tanpa email'}</small></label>)}</div>}
  ...
  return <div className="admin-panel-overlay" onMouseDown={onClose}>
    <aside className="admin-detail detail-readable" onMouseDown={event => event.stopPropagation()}>
      ...
      {tab === 'tasks' && <>
        ...
        <section className="detail-section"><h3>Tugas khusus admin</h3>{(detail?.assignments || []).map(item => <div className="task-line" key={item.id}><i className={`fas ${item.status === 'completed' ? 'fa-circle-check text-emerald-500' : item.status === 'needs_retake' ? 'fa-rotate text-amber-500' : 'fa-clipboard-check text-[var(--accent-strong)]'}`} /><div className="flex-1"><b>{item.title}</b><small>{item.target.packTitle || item.target.title || item.target.topic || item.target.theme || 'Tugas terarah'} · {item.status === 'completed' ? 'Target tercapai' : item.status === 'needs_retake' ? 'Perlu retake' : 'Belum tercapai'}</small></div>{item.status === 'completed' && <button className="delete-text" onClick={() => onRetakeAssignment(item.id)}>Beri retake</button>}</div>)}{!(detail?.assignments || []).length && <p className="text-[12px] text-[var(--muted)]">Belum ada tugas khusus dari admin.</p>}</section>
