  ...
  return <div className={`admin-root admin-${theme} admin-accent-${accent}`}>
    ...
    <div className="admin-layout">
      ...
      <main className="admin-main">
        <div className="admin-mobile-bar">{navItems.map(item => <button key={item.id} onClick={() => setSection(item.id)} className={section === item.id ? 'active' : ''}>{item.label}{item.count !== undefined ? ` · ${item.count}` : ''}</button>)}<button className="admin-mobile-return" onClick={() => window.location.assign('/')}><i className="fas fa-arrow-left mr-1" />LovSpeak</button></div>
